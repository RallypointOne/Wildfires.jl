...
_cpu_moisture(m::SpreadModels.AbstractMoisture) = m
function _cpu_moisture(m::SpreadModels.DynamicMoisture)
    SpreadModels.DynamicMoisture(
...
function _cpu_model(model::SpreadModels.RothermelModel)
    SpreadModels.RothermelModel(
...
function _gpu_spread_rate_field!(F, model, grid)
    ...
    cm = _cpu_model(model)
    SpreadModels.spread_rate_field!(F_cpu, cm, cpu_grid)
    copyto!(F, F_cpu)
    F
end

# Generic model dispatch
function SpreadModels.spread_rate_field!(F::AbstractGPUArray{T, 2}, model, grid::LevelSetGrid) where {T}
    _gpu_spread_rate_field!(F, model, grid)
end

# RothermelModel dispatch (resolves ambiguity with base RothermelModel method)
function SpreadModels.spread_rate_field!(F::AbstractGPUArray{T, 2}, model::SpreadModels.RothermelModel, grid::LevelSetGrid) where {T}
    _gpu_spread_rate_field!(F, model, grid)
end

#-----------------------------------------------------------------------------# GPU simulate!

function SpreadModels.simulate!(grid::LevelSetGrid{T, <:AbstractGPUArray}, model;
    ...
    bo = SpreadModels._coerce_burnout(burnout)
    bi = SpreadModels._coerce_burnin(burnin)
    F_gpu = similar(grid.φ)

    for step in 1:steps
        SpreadModels.spread_rate_field!(F_gpu, model, grid)
        ...
        SpreadModels.update!(model, grid, step_dt)
        LevelSet.advance!(grid, F_gpu, step_dt, solver)
        step % reinit_every == 0 && LevelSet.reinitialize!(grid, reinit)
        trace !== nothing && step % trace.every == 0 && SpreadModels._record!(trace, grid)
        progress && step % max(1, steps ÷ 100) == 0 && SpreadModels._print_progress(step, steps, grid)
    end
    progress && println()
    grid
end

_gpu_scale_burnout!(F, grid, ::SpreadModels.NoBurnout) = nothing
...
function _gpu_scale_burnout!(F, grid, bo::SpreadModels.ExponentialBurnout)
...
function _gpu_scale_burnout!(F, grid, bo::SpreadModels.LinearBurnout)
    backend = get_backend(F)
    ny, nx = size(F)
...
_gpu_scale_burnin!(F, grid, ::SpreadModels.NoBurnin) = nothing
...
function _gpu_scale_burnin!(F, grid, bi::SpreadModels.ExponentialBurnin)
    backend = get_backend(F)
    ny, nx = size(F)
...
function _gpu_scale_burnin!(F, grid, bi::SpreadModels.LinearBurnin)
    backend = get_backend(F)
    ny, nx = size(F)
