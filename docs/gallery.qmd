...
for dir in [CosineBlending(), EllipticalBlending()]
    ...
    model = RothermelModel(SHORT_GRASS, UniformWind(speed=8.0), UniformMoisture(M), FlatTerrain(), dir)
    tr = Trace(g, 3)
...
for (idx, (dir, label)) in enumerate(zip(directions, labels))
    ...
    model = RothermelModel(SHORT_GRASS, UniformWind(speed=8.0, direction=dir),
...
g = LevelSetGrid(100, 100, dx=30.0)
ignite!(g, 1500.0, 1500.0, 50.0)
model = RothermelModel(SHORT_GRASS, UniformWind(speed=0.5), UniformMoisture(M), FlatTerrain())
tr = Trace(g, 3)
...
model = RothermelModel(CHAPARRAL, UniformWind(speed=8.0), UniformMoisture(M),
    FlatTerrain(), EllipticalBlending())
tr = Trace(g, 3)
simulate!(g, model, steps=150, dt=0.5, trace=tr, burnout=ExponentialBurnout(t_r))
...
for s in slopes
    g = LevelSetGrid(100, 100, dx=30.0)
    ignite!(g, 1500.0, 1500.0, 50.0)
    model = RothermelModel(SHORT_GRASS, UniformWind(speed=5.0), UniformMoisture(M),
...
model_d = RothermelModel(SHORT_GRASS, UniformWind(speed=8.0), dm, FlatTerrain())
tr_d = Trace(g_d, 3)
...
g = LevelSetGrid(150, 150, dx=30.0)
ignite!(g, 2250.0, 2250.0, 50.0)
model = RothermelModel(SHORT_GRASS, UniformWind(speed=8.0), UniformMoisture(M),
    FlatTerrain(), EllipticalBlending())
...
for (idx, (fuel, label)) in enumerate(zip(fuels, fuel_labels))
    ...
    g = LevelSetGrid(100, 100, dx=30.0)
    ignite!(g, 1500.0, 1500.0, 50.0)
    model = RothermelModel(fuel, UniformWind(speed=8.0), UniformMoisture(M),
        FlatTerrain(), EllipticalBlending())
...
Elliptical spread with burnout on chaparral fuel over real-world terrain (Thomas Fire origin, Anlauf Canyon, CA) with wind particles — also displayed on the [landing page](index.qmd).
