...
The `spread_rate_field!` function evaluates the `RothermelModel` at each cell, which involves branching logic in the Rothermel model that isn't GPU-friendly.  The extension handles this transparently: it copies `φ` to the CPU, computes `F`, and uploads the result back to the GPU.
