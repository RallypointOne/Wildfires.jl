...
Each entry in `trace.stack` is a `(time, φ)` tuple.  Here's how to create a GIF:
...
record(fig, joinpath(@__DIR__, "fire_spread.gif"), eachindex(trace.stack); framerate=15) do idx
    empty!(ax)
    t, φ = trace.stack[idx]
    ax.title = @sprintf("t = %.1f min", t)
    contourf!(ax, xs, ys, φ, levels=[-1e4, 0, 1e4],
        colormap=[:red, :green])
end
```
