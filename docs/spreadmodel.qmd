...
The `LevelSetGrid` includes a `burnable` field — a `BitMatrix` where `false` marks cells that fire cannot enter.  By default all cells are burnable.  Use `set_unburnable!` to mark circular regions, or directly set `grid.burnable[i, j] = false` for arbitrary shapes.
