...
# Spread rate: faster on the right side (e.g. wind or fuel gradient)
F = [5.0 + 10.0 * (j / 200) for i in 1:200, j in 1:200]
...
for (col, target_t) in enumerate(times)
    ...
    ax = Axis(fig[1, col], title="t = $(round(g.t, digits=1)) min", aspect=DataAspect())
    fireplot!(ax, g)
    hidedecorations!(ax)
end
fig
