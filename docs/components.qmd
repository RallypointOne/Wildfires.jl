...
The `Components` module defines the abstract interfaces and concrete types used by `SpreadModels` to represent wind, moisture, terrain, burnout, burn-in, and directional spread.  Each component is a callable struct with signature `(t, x, y)` that returns the relevant physical quantity at a given time and location.
...
| `AbstractBurnout` | `burnout(t_burning)` | `Float64 ∈ [0, 1]` — spread intensity decay |
| `AbstractBurnin` | `burnin(t_burning)` | `Float64 ∈ [0, 1]` — spread intensity ramp-up |
| `AbstractBlendingMode` | *(used internally by `RothermelModel`)* | Directional interpolation of spread rate |
| `AbstractSpotting` | `spot!(grid, model, dt)` | Ignites new spot fires from lofted embers |
| `AbstractSuppression` | `suppress!(grid, model, dt)` | Modifies grid to reflect suppression actions |

::: {.callout-note}
`AbstractSpotting` and `AbstractSuppression` are defined but not yet connected to `simulate!`. They are reserved for future use.
:::
...
## Directional Spread (Blending Modes)

Blending modes (`AbstractBlendingMode`) control how the spread rate is interpolated across angles relative to the push direction (combined wind + slope vector). Given the head-fire rate `R_head` (maximum, aligned with push) and a base rate `R_base` (no wind/slope), the blending mode computes the effective spread rate at any angle `θ` from the push direction.
...
m_cos = RothermelModel(SHORT_GRASS, UniformWind(speed=10.0), UniformMoisture(M), FlatTerrain())
simulate!(g_cos, m_cos, steps=200)

g_ell = LevelSetGrid(150, 150, dx=30.0)
ignite!(g_ell, 2250.0, 2250.0, 200.0)
m_ell = RothermelModel(SHORT_GRASS, UniformWind(speed=10.0), UniformMoisture(M), FlatTerrain(), EllipticalBlending())
...
#| code-fold: false
#| eval: false
function SpreadModels.update!(w::MyWindField, grid::LevelSetGrid, dt)
