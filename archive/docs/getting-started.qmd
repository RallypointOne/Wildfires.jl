...
The `SpreadModels` module lets you compose a `RothermelModel` from pluggable wind, moisture, and terrain components:
...
- [Rothermel](rothermel.qmd) — fuel models, parameter sensitivity, and the physics behind rate of spread
- [Level Set](levelset.qmd) — boundary conditions, reinitialization, plotting options, and the numerical scheme
- [Spread Model](spreadmodel.qmd) — dynamic moisture, custom wind/terrain components, and simulation parameters
- [GPU](gpu.qmd) — running simulations on CUDA, ROCm, or Metal backends
- [PINN Solver](pinn.qmd) — mesh-free fire spread via physics-informed neural networks
- [API Reference](api.qmd) — full documentation of all exported functions and types
