...
The `SpreadModels` module provides composable, differentiable components for driving level set fire simulations.  Instead of manually constructing a spread rate matrix `F`, you build a `RothermelModel` from pluggable wind, moisture, and terrain components — each a callable `(t, x, y)` struct.

## Architecture

```
RothermelModel(fuel, wind, moisture, terrain)
       │
       ▼
...
By default, `RothermelModel` uses cosine blending to distribute the head-fire rate across angles.  This produces fires that are wider than typically observed.  The **elliptical model** (Anderson, 1983) produces more realistic elongated fire shapes by computing a length-to-breadth ratio from wind speed.

Pass a directional model as the 5th argument to `RothermelModel`:
...
for (col, (label, dir)) in enumerate([
    ...
    m = RothermelModel(SHORT_GRASS, UniformWind(speed=8.0), UniformMoisture(M), FlatTerrain(), dir)
...
model_s = RothermelModel(SHORT_GRASS, UniformWind(speed=8.0), UniformMoisture(M), FlatTerrain())
simulate!(grid_s, model_s, steps=150, dt=0.5)

# Dynamic
grid_d = LevelSetGrid(150, 150, dx=20.0)
ignite!(grid_d, 1500.0, 1500.0, 50.0)
model_d = RothermelModel(SHORT_GRASS, UniformWind(speed=8.0), DynamicMoisture(grid_d, M), FlatTerrain())
...
for (col, s) in enumerate([0.0, 0.3, 0.6])
    ...
    m = RothermelModel(SHORT_GRASS, UniformWind(speed=5.0), UniformMoisture(M), UniformSlope(slope=s))
...
ignite!(g1, 4500.0, 3000.0, 50.0)
simulate!(g1, model, steps=200, dt=0.5)
ax1 = Axis(fig[1, 1], title="No fuel break", aspect=DataAspect())
fireplot!(ax1, g1)
hidedecorations!(ax1)

# With fuel break (strip in the fire's path)
g2 = LevelSetGrid(200, 200, dx=30.0)
ignite!(g2, 4500.0, 3000.0, 50.0)
# Mark columns as unburnable ahead of the fire (lower x = south on plot)
xs = xcoords(g2)
for j in eachindex(xs)
    if 3000.0 <= xs[j] <= 3150.0
        g2.t_ignite[:, j] .= NaN
    end
end
...
fireplot!(ax2, g2)
hspan!(ax2, 3000.0, 3150.0, color=(:black, 0.15))
hidedecorations!(ax2)
fig
```
...
m1 = RothermelModel(SHORT_GRASS, UniformWind(speed=8.0), UniformMoisture(M), FlatTerrain())
...
g = LevelSetGrid(150, 150, dx=20.0)
ignite!(g, 1500.0, 1500.0, 50.0)
m = RothermelModel(SHORT_GRASS, GradientWind(5.0, 0.003), UniformMoisture(M), FlatTerrain())
simulate!(g, m, steps=150, dt=0.5)
...
```julia
SpreadModels.update!(w::MyDynamicWind, grid::LevelSetGrid, dt) = ...
```
...
| `solver` | `Godunov()` | Numerical solver for the level set equation.  See [Solvers](levelset.qmd#solvers). |
| `curvature` | `0.0` | Curvature regularization coefficient.  When > 0, smooths the fire front via `F_eff = max(F - b·κ, 0)`.  See [Curvature](levelset.qmd#curvature-regularization). |
| `reinit` | `IterativeReinit()` | Reinitialization method.  `IterativeReinit()` (default) or `NewtonReinit()` for machine-precision signed distance.  See [Reinitialization](levelset.qmd#reinitialization). |

**`solver`**: Controls the numerical scheme for advancing the level set.  `Godunov()` (default) uses first-order upwind differencing with forward Euler.  `Superbee()` uses a second-order Superbee flux limiter with RK2 (Heun's method) for sharper fire fronts.  `WENO5()` uses a 5th-order WENO scheme with SSP-RK3 for the highest accuracy.  See the [Solvers](levelset.qmd#solvers) section for details.
